  ...
  return (
    <Modal title={editing ? 'Editar curso' : 'Nuevo curso'} kicker="// Mundo" onClose={close}>
      <form onSubmit={submit} className="form">
        <Field label="Nombre del curso">{(fid) => <input id={fid} className="input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Análisis de Datos" required maxLength={80} />}</Field>
        <div className="form__row">
          <Field label="Profesor/a">{(fid) => <input id={fid} className="input" value={professor} onChange={(e) => setProfessor(e.target.value)} placeholder="Prof. Marta Núñez" />}</Field>
          <Field label="Área">{(fid) => <input id={fid} className="input" value={field} onChange={(e) => setField(e.target.value)} placeholder="ciencia de datos" />}</Field>
        </div>
        {!editing && (
        ...
        <fieldset className="fieldset">
          ...
          <div className="form__row">
            <Field label="Nombre">{(fid) => <input id={fid} className="input" value={mentorName} onChange={(e) => setMentorName(e.target.value)} />}</Field>
            <Field label="Especialidades">{(fid) => <input id={fid} className="input" value={mentorSkills} onChange={(e) => setMentorSkills(e.target.value)} placeholder="SQL · Python" />}</Field>
          </div>
  ...
  return (
    <Modal title="Nueva meta" kicker="// Árbol de habilidades" onClose={close}>
      <form onSubmit={submit} className="form">
        ...
        <Field label="Recompensa final">{(fid) => <input id={fid} className="input" value={rewardTitle} onChange={(e) => setRewardTitle(e.target.value)} placeholder="Título: Analista de Datos Jr." />}</Field>
...
export function RewardModal() {
  ...
  return (
    <Modal title="Recompensa personal" kicker="// Premio de la vida real" onClose={close}>
      <form onSubmit={submit} className="form">
        <Field label="Premio">{(fid) => <input id={fid} className="input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ver una película sin culpa" required maxLength={100} />}</Field>
        <Field label="Condición">{(fid) => <input id={fid} className="input" value={condition} onChange={(e) => setCondition(e.target.value)} placeholder="Si completo 5 días seguidos de estudio" />}</Field>
