...
import { AiError, DEFAULT_MODEL, cleanKey, generateCards, generateRoadmap, looksLikeKey, maskKey, testKey, type GenAiClient } from './gemini';
...
describe('almacén de la clave', () => {
  ...
  it('migra el modelo por defecto anterior al actual y respeta el que el usuario haya elegido', () => {
    const old = new MemStore();
    old.setItem('sq:ai', JSON.stringify({ apiKey: KEY, model: 'gemini-2.5-flash' }));
    expect(createAiStore(old).getState().model).toBe(DEFAULT_MODEL);
    const custom = new MemStore();
    custom.setItem('sq:ai', JSON.stringify({ apiKey: KEY, model: 'gemini-3.5-flash-lite' }));
    expect(createAiStore(custom).getState().model).toBe('gemini-3.5-flash-lite');
  });
